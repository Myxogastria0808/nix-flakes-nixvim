...
{ pkgs, ... }:
{
  ...
  #
  # LSP capability guard — keymaps are guarded by lspsaga_guard() which
  # checks server_capabilities before sending a request to avoid leaving
  # lspsaga in a stuck "request pending" state.
  #
  # Keybind | Required capability       | Guard
  # --------|---------------------------|------
  # K       | hoverProvider             | yes
  # gp      | definitionProvider        | yes
  # gd      | definitionProvider        | yes
  # gP      | typeDefinitionProvider    | yes
  # gT      | typeDefinitionProvider    | yes
  # <A-f>   | referencesProvider        | yes
  # <A-r>   | renameProvider            | yes
  # <A-a>   | codeActionProvider        | yes
  # <A-O>   | documentSymbolProvider    | yes
  # <A-i>   | callHierarchyProvider     | yes
  # <A-o>   | callHierarchyProvider     | yes
  # gi      | implementationProvider    | yes
  # <A-e>   | (always available)        | no
  # <A-E>   | (always available)        | no
  # ]d      | (always available)        | no
  # [d      | (always available)        | no
  # <A-t>   | (always available)        | no
  ...
  # All LSP-dependent keymaps are guarded by lspsaga_guard (defined in
  # extraConfigLua above). If the current buffer's LSP server does not
  # advertise the required capability, a warning notification is shown
  # instead of sending a request that could leave lspsaga stuck.
  # Diagnostics and terminal keymaps do not depend on LSP capabilities.
  keymaps = [
    ...
    # keybind: K
    {
      mode = "n";
      key = "K";
      action.__raw = "function() lspsaga_guard('hoverProvider', 'Lspsaga hover_doc', 'K (hover documentation)') end";
    ...
    # keybind: Alt + F
    {
      mode = "n";
      key = "<A-f>";
      action.__raw = "function() lspsaga_guard('referencesProvider', 'Lspsaga finder', 'Alt+F (finder)') end";
    ...
    # keybind: gp
    {
      mode = "n";
      key = "gp";
      action.__raw = "function() lspsaga_guard('definitionProvider', 'Lspsaga peek_definition', 'gp (peek definition)') end";
    ...
    # keybind: gd
    {
      mode = "n";
      key = "gd";
      action.__raw = "function() lspsaga_guard('definitionProvider', 'Lspsaga goto_definition', 'gd (go to definition)') end";
    ...
    # keybind: gP
    {
      mode = "n";
      key = "gP";
      action.__raw = "function() lspsaga_guard('typeDefinitionProvider', 'Lspsaga peek_type_definition', 'gP (peek type definition)') end";
    ...
    # keybind: gT
    {
      mode = "n";
      key = "gT";
      action.__raw = "function() lspsaga_guard('typeDefinitionProvider', 'Lspsaga goto_type_definition', 'gT (go to type definition)') end";
    ...
    # keybind: Alt + R
    {
      mode = "n";
      key = "<A-r>";
      action.__raw = "function() lspsaga_guard('renameProvider', 'Lspsaga rename', 'Alt+R (rename)') end";
    ...
    # keybind: Alt + A
    {
      ...
      key = "<A-a>";
      action.__raw = "function() lspsaga_guard('codeActionProvider', 'Lspsaga code_action', 'Alt+A (code action)') end";
    ...
    # keybind: Alt + Shift + O
    {
      mode = "n";
      key = "<A-O>";
      action.__raw = "function() lspsaga_guard('documentSymbolProvider', 'Lspsaga outline', 'Alt+Shift+O (outline)') end";
    ...
    # keybind: Alt + I
    {
      mode = "n";
      key = "<A-i>";
      action.__raw = "function() lspsaga_guard('callHierarchyProvider', 'Lspsaga incoming_calls', 'Alt+I (incoming calls)') end";
    ...
    # keybind: Alt + O
    {
      mode = "n";
      key = "<A-o>";
      action.__raw = "function() lspsaga_guard('callHierarchyProvider', 'Lspsaga outgoing_calls', 'Alt+O (outgoing calls)') end";
    ...
    # keybind: gi
    {
      mode = "n";
      key = "gi";
      action.__raw = "function() lspsaga_guard('implementationProvider', 'Lspsaga finder imp', 'gi (show implementations)') end";
