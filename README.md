...
| **Dashboard**      | [alpha-nvim](https://github.com/goolord/alpha-nvim)                                                                                                   | Start screen with geometric ASCII art logo and Neovim version                                                                                                                                  |
| **File tree**      | [neo-tree.nvim](https://github.com/nvim-neo-tree/neo-tree.nvim)                                                                                       | Git status icons, auto-close on last window, follow current file; `open_files_do_not_replace_types` prevents neo-tree / quickfix / sagaoutline from being overwritten on file open             |
| **Icons**          | [nvim-web-devicons](https://github.com/nvim-tree/nvim-web-devicons)                                                                                   | Custom icons for `.lean` (∀), `lean-toolchain` (∃), `.typ` (𝐭), `.agda` (󱗆), `.mmd` (󰒪), `.dot` (󱁉), `.envrc/.bashrc/.zshrc` ($)                                                               |
...
| **Syntax**         | [nvim-treesitter](https://github.com/nvim-treesitter/nvim-treesitter)                                                                                 | 38-parser set — highlighting + indent                                                                                                                                                          |
...
| **Formatting**     | [conform.nvim](https://github.com/stevearc/conform.nvim)                                                                                              | Format-on-save (2000 ms timeout); trailing newline enforced on every save — see [Language Support](#language-support)                                                                          |
| **Linting**        | [nvim-lint](https://github.com/mfussenegger/nvim-lint)                                                                                                | [actionlint](https://github.com/rhysd/actionlint) (GitHub Actions), [hadolint](https://github.com/hadolint/hadolint) (Dockerfile), [checkmake](https://github.com/mrtazz/checkmake) (Makefile) |
...
| **Preview**        | [markdown-preview.nvim](https://github.com/iamcco/markdown-preview.nvim), [typst-preview.nvim](https://github.com/chomosuke/typst-preview.nvim)       | Live Markdown + Mermaid preview in browser; live Typst preview in browser                                                                                                                      |
...
| **Undo history**   | [telescope-undo.nvim](https://github.com/debugloop/telescope-undo.nvim)                                                                               | Browse undo history in a floating window with live diff preview (`Alt+u`)                                                                                                                      |
| **Crash recovery** | Persistent undo (`undofile`)                                                                                                                          | Swap files disabled; undo history persists across sessions via `~/.local/state/nvim/undo/` — reopen a file and `:u` to restore previous states                                                 |
...
| TypeScript          | ts_ls                           | oxfmt                            | ✓          |                                                                                                                                                                                                                                                                                  |
| JavaScript          | ts_ls                           | oxfmt                            | ✓          |                                                                                                                                                                                                                                                                                  |
| TSX / JSX           | ts_ls                           | oxfmt                            | ✓          | Context-aware commenting via ts-context-commentstring                                                                                                                                                                                                                            |
...
| Typst               | tinymist                        | typstyle                         | ✓          | Live browser preview via [typst-preview.nvim](https://github.com/chomosuke/typst-preview.nvim)                                                                                                                                                                                   |
| HTML                | html                            | oxfmt                            | ✓          |                                                                                                                                                                                                                                                                                  |
| CSS                 | cssls                           | oxfmt                            | ✓          |                                                                                                                                                                                                                                                                                  |
| JSON                | jsonls                          | oxfmt                            | ✓          |                                                                                                                                                                                                                                                                                  |
| YAML                | yamlls                          | oxfmt                            | ✓          | GitHub Actions schema auto-applied to `.github/workflows/*.yml`                                                                                                                                                                                                                  |
| TOML                | taplo                           | taplo                            | ✓          |                                                                                                                                                                                                                                                                                  |
...
| Mermaid             | —                               | mermaidfmt                       | ✓          | `.mmd` filetype registered; no Mermaid LSP exists. [mermaid-formatter](https://github.com/chenyanchen/mermaid-formatter) built from npm tarball via `stdenv.mkDerivation`                                                                                                        |
| Graphviz DOT        | dotls                           | dot_canon (graphviz)             | ✓          | `.dot` filetype; `dot -Tcanon` reformats to canonical DOT syntax. [dot-language-server](https://github.com/nikeee/dot-language-server) for diagnostics and completions.                                                                                                          |
...
> [!NOTE]
> **Trailing newline enforcement** — On every save, a `BufWritePre` autocmd (registered in `config/libs/language/format.nix`) runs after conform.nvim and ensures the buffer ends with exactly one newline:
>
> 1. Any consecutive blank lines at the end of the file are collapsed down to one.
> 2. If the last line is non-empty, a blank line is appended so the file always terminates with `\n`.
>
> This applies to every filetype, regardless of whether a formatter is configured.
...
| Key               | Mode                                | Action                                         |
| ----------------- | ----------------------------------- | ---------------------------------------------- |
| `u`               | Normal                              | Undo (persists across sessions via `undofile`) |
| `Ctrl+R`          | Normal                              | Redo (persists across sessions via `undofile`) |
| `Alt+u`           | Normal                              | Open undo history (telescope floating window)  |
...
> 3. **Two-phase guard** (`config/libs/ui/tree.nix`): `argv()` is scanned for directory arguments at `init.lua` time (Phase 1), and each pre-created buffer is immediately marked `buftype=nofile` to block the file-read that generates the error messages. `:argdelete` is intentionally deferred to a `VimEnter` autocmd (Phase 2) so that alpha-nvim's earlier `VimEnter` still sees `argc() > 0` and skips the startup dashboard — preventing a brief flash of the dashboard before neo-tree opens. Once the deferred `VimEnter` fires, `:argdelete` is called for each directory, and if all arguments were directories Neovim `cd`s into the first one and opens neo-tree.
>
> The guard also fixed a secondary bug: `pkgs.vimPlugins.cornelis` ships no Lua module, so `require("cornelis")` in `config/libs/language/lsp.nix` was an uncaught error that terminated `init.lua` before the guard code could run. Wrapping it in `pcall` ensures `init.lua` always executes completely.
...
| `Alt+o` | Normal            | Show outgoing call hierarchy                        |
...
### Preview (markdown-preview.nvim / typst-preview.nvim)
...
- Supported platforms: `x86_64-linux` (manually verified), `aarch64-darwin` / `x86_64-darwin` (declared but unverified — no macOS machine available for testing)
...
│       ├── language/
│       │   ├── lsp.nix              # LSP servers (28 languages) + lean.nvim + cornelis (Agda)
│       │   ├── lspsaga.nix          # lspsaga.nvim (rich LSP UI + float terminal)
│       │   ├── format.nix           # conform.nvim (format-on-save) + nvim-lint + mermaid-formatter
│       │   └── treesitter.nix       # Syntax highlighting (nvim-treesitter, 38 parsers)
│       └── action/
